...
public class YeelightDevice {
    ...
    private static ArrayList<Integer> portsUsed = new ArrayList<>();
     ...
     */
    public YeelightDevice(String ip, int port, YeelightEffect effect, int duration) throws YeelightSocketException, YeelightResultErrorException {
        this.socketHolder = new YeelightSocketHolder(ip, port);
        this.setEffect(effect);
        this.setDuration(duration);

        int randomPort = generateRandomServerPort();
        this.server = new TCPServer(randomPort);
        server.start();
        this.sendCommand(new YeelightCommand("set_music", 1, "192.168.1.9", randomPort));
        this.setPower(true);
    }
     ...
     * @throws YeelightSocketException when a socket error occurs
     */
    public YeelightDevice(String ip, int port) throws YeelightSocketException, YeelightResultErrorException {
     ...
     * @throws YeelightSocketException when a socket error occurs
     */
    public YeelightDevice(String ip) throws YeelightSocketException, YeelightResultErrorException {
        this(ip, 55443);
    }
    ...
    private void sendTCPCommand(YeelightCommand command) {
    ...
    private int generateRandomServerPort() {
        boolean firstRun = true;
        boolean numValid = true;
        Random rand = new Random();
        int randomPort = rand.nextInt(9000) + 1000;
        while(firstRun || !numValid) {
            firstRun = false;
            for (int n : portsUsed) {
                if (randomPort == n) {
                    numValid = false;
        ...
        portsUsed.add(randomPort);
        return randomPort;
    }

    public void stopServer() throws YeelightResultErrorException, YeelightSocketException {
        setPower(false);
        server.stopServer();
    }
}
